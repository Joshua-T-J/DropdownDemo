/**
 * MaterialDropdownAdapter — stub / reference implementation.
 */
...
@Injectable()
export class MaterialDropdownAdapter implements DropdownAdapter<any> {
  ...
  init(options: DropdownAdapterOptions): void {
    ...
    this._matSelect.selectionChange
      ...
      .subscribe((event) => {
        const selectedItem = options.itemsSource?.find(
          (item, index) =>
            componentRef.instance.resolveValue(item) === event.value
            && index === event.source.options.toArray().findIndex((option) => option.value === event.value)
        ) ?? options.itemsSource?.find((item) => componentRef.instance.resolveValue(item) === event.value) ?? null;
  ...
  setReadOnly(isReadOnly: boolean): void {
    // MatSelect does not support readOnly natively; fallback to disabling
    this.setDisabled(isReadOnly);
    if (this._componentRef && this._componentRef.location && this._componentRef.location.nativeElement) {
      if (isReadOnly) {
        this._componentRef.location.nativeElement.style.pointerEvents = 'none';
      } else {
        this._componentRef.location.nativeElement.style.pointerEvents = '';
      }
    }
  }
